...
import net.mythigame.commonsMGC.Kronos.managers.UnbanManager;
...
public class unbanCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(sender instanceof Player moderator){
            ...
            if(account != null){
                ...
                Account targetAccount = new Account().getAccount(MojangAPI.getUUIDFromUsername(args[0]));
                String reason = "";

                for(int i = 1; i < args.length; i++){
                    reason += args[i] + " ";
                }
                UnbanManager unbanManager = new UnbanManager(targetAccount, moderator, reason);
                unbanManager.unban();
            }
        }
    ...
    private void helpMessage(CommandSender sender){
        sender.sendMessage("§c[Kronos] Usage : /unban <player> <reason>");
    }
