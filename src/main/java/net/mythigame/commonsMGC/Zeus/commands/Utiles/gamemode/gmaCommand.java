...
public class gmaCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(args.length == 0){
            ...
            player.sendMessage("§9[Zeus] Vous êtes désormais en gamemode aventure.");
            ...
            target.sendMessage("§9[Zeus] Vous êtes désormais en gamemode aventure.");
            player.sendMessage("§9[Zeus] " + target.getName() + " est désormais en gamemode aventure.");
        }else helpMessage(player);
        return true;
    }

    private void helpMessage(CommandSender sender){
        sender.sendMessage("§9[Zeus] Usage : /gma [(<player>)]");
    }
